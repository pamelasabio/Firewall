...
unsigned int hook_func_in(void *priv, struct sk_buff *skb, const struct nf_hook_state *state){	
	...
	}else if ((ip_header->protocol == PROTOCOL_HTTP) || (ip_header->protocol == PROTOCOL_HTTPS)){
		printk(KERN_INFO "Bingo! HTTP/S PROTOCOL!");
	}else{
		printk(KERN_INFO "Protocol: %d", ip_header->protocol);
	}
